...
There are no transactions in MongoDB, but you can use technique 2-phase commit.

## Object ID

12 bytes, 24 charaters in total.

- 4 bytes: timestamp when document is created => do not need CreatedAt property
- 3 bytes: machine identifier
- 2 bytes: process identifier
- 3 bytes: counter

> Theoretical threshold of counter is 2 ^ 24 = 16M records, that can be at the same time on same machine and process. Then theoretically counter can overflow.

> ID is generated by driver, not MongodDB itself. Therefore, apps are highly scalable. You do not need to wait for Mongo to give ID in return after insert.

To generate new ID explicitly:
